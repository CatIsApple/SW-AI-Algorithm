...
"""
def binary_search(arr, arr2, target):

    if len(arr) == 0:
        return -1

    arrLen = len(arr) // 2 #index
    ...
    if mid < target:
        return binary_search(right, arr2, target)

    elif mid > target:
        return binary_search(left, arr2, target)

    else:
        return arr2.index(mid)
# 테스트 케이스
if __name__ == "__main__":
    ...
    target1 = 7
    result1 = binary_search(arr1, arr1, target1)
    print(f"배열: {arr1}")
    ...
    target2 = 14
    result2 = binary_search(arr2, arr2, target2)
    print(f"배열: {arr2}")
    ...
    arr3 = [1, 3, 5, 7, 9]
    target3 = 6
    result3 = binary_search(arr3, arr3, target3)
    print(f"배열: {arr3}")
